...
public class EmailValidation implements IUserValidation {
    ...
    public Mono<Void> validate(User user) {
        if (user.getEmail() == null || user.getEmail().isEmpty()) {
            return Mono.error(new UserValidationException("El campo email es obligatorio."));
        }
        String emailRegex = "^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@" +
                "(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$";
        if (!user.getEmail().matches(emailRegex)) {
            return Mono.error(new UserValidationException("El campo email debe tener un formato válido."));
        }
        return userRepository.findByEmail(user.getEmail())
                .flatMap(existing -> Mono.error(new UserValidationException("El email indicado ya existe, intente con otro.")))
                .then();
    }
}
